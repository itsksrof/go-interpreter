...
func TestFunctionObject(t *testing.T) {
	...
	if fn.Parameters[0].String() != "x" {
		t.Fatalf("parameter is not %s. got=%q", "x", fn.Parameters[0])
	}
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		{`first("array")`, "argument to 'first' must be ARRAY, got=STRING"},
		{`first([])`, nil},
		{`last([1,2,3])`, 3},
		{`last()`, "wrong number of arguments. got=0, want=1"},
		{`last("array")`, "argument to 'last' must be ARRAY, got=STRING"},
		{`last([])`, nil},
		{`rest([1,2,3])`, []int{2, 3}},
		{`rest()`, "wrong number of arguments. got=0, want=1"},
		{`rest("array")`, "argument to 'rest' must be ARRAY, got=STRING"},
		{`rest([])`, nil},
		{`push([1,2,3], 4)`, []int{1, 2, 3, 4}},
		{`push()`, "wrong number of arguments. got=0, want=2"},
		{`push("array", 1)`, "argument to 'push' must be ARRAY, got=STRING"},
	}
