...
// evalHashIndexExpression checks if the given index implements the object.Hashable
// interface, if it does not returns an error, otherwise looks for the pairs inside the
// hash object using the hash key, and if it's not empty returns the pair value, otherwise
// returns NULL.
